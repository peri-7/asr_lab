...
class SentenceDataset(Dataset):
    ...
    def __init__(self, X, y, word2idx):
        ...
        self.raw = X
        self.data = [tk.tokenize(clean_text(sentence)) for sentence in X]
        self.labels = y
        ...
        # code to be commented out to see the ditribution of sentence length
        '''
        ...
        for sentence in self.data:
            ...
            if length < N_max:
                padding = [0] * (N_max-length)
                idx_tokens.extend(padding)
            #store list of indices
            self.data_nn.append(idx_tokens)
